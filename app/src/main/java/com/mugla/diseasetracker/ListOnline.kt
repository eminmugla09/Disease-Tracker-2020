...
import com.mugla.diseasetracker.model.Tracking
import com.mugla.diseasetracker.model.User
...
class ListOnline : AppCompatActivity(),GoogleApiClient.ConnectionCallbacks,
    ...
    //firebase connection and import and export the date.
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        sharedPreferences=getSharedPreferences("DiseaseTracker",Context.MODE_PRIVATE)
        toolbar=findViewById(R.id.toolbar)
        setSupportActionBar(toolbar)
        //toolbar title
        supportActionBar?.title="Disease Tracker"
        ...
        //gettin information from firebase
        if(intent!=null) {
    ...
    //firebase location latitude and longtitue
    ...
    //this function will show who online and also visible to screen.
    fun updateList() {
        ...
        adapter=object : FirebaseRecyclerAdapter<User, ListOnlineViewHolder>(options){
            ...
            // this fuction shows name of the users on the list
            override fun onBindViewHolder(p0: ListOnlineViewHolder, p1: Int, p2: User) {
                ...
                p0.llContent.setOnClickListener {
                    ...
                    if(!p2.email.equals(FirebaseAuth.getInstance().currentUser!!.email)){
                        ...
                        val map=Intent(this@ListOnline,
                            MapTracking::class.java)
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when(item.itemId){
            R.id.action_invite ->{
                if(nameOfGroup==null){
                    Toast.makeText(this,"First join a Quarantine Zone",Toast.LENGTH_SHORT).show()
                }
                else{
                    ...
                    try{
                        ...
                        val text="Download Disease Tracker app\n And Join the Quarantine zone with this Quarentine zone name : "+nameOfGroup
            ...
            R.id.action_join -> {
                ...
                alert.setTitle("Join an existing Quarantine Zone")
                alert.setMessage("Enter Quarantine Zone Name")
...
//                                onlineRef=FirebaseDatabase.getInstance().getReference(nameOfGroup).child(".info/connected")
            ...
            R.id.action_left -> {
                ...
                alert.setMessage("Exit Quarantine Zone")
                alert.setPositiveButton("OK"){ dialogInterface: DialogInterface, i: Int ->
                    ...
                    try {
                        ...
                        Toast.makeText(this,"First Join an Quarantine Zone",Toast.LENGTH_SHORT).show()
                    }
                }
            ...
            R.id.action_create ->{
                ...
                val alert=AlertDialog.Builder(this)
                alert.setTitle("Create a Quarentine Zone")
                alert.setMessage("Enter Name to Quarantine Zone")
                val input= EditText(this)
                alert.setView(input)
                alert.setPositiveButton("OK") { dialog, which ->
                    val nameOfGroup=input.text.toString()
                    ...
                    groups.child(nameOfGroup).addListenerForSingleValueEvent(object:ValueEventListener{
                        ...
                        override fun onDataChange(p0: DataSnapshot) {
                            if(p0.value==null){
                                ...
                                val sharedPreferences=getSharedPreferences("DiseaseTracker", Context.MODE_PRIVATE)
                                sharedPreferences.edit().putString("nameOfGroup",nameOfGroup)
                                locations=FirebaseDatabase.getInstance().getReference(nameOfGroup)
                                ...
                                Toast.makeText(this@ListOnline,"Quarantine Zone Name Already Exist",Toast.LENGTH_SHORT).show()
                            }
                        }
